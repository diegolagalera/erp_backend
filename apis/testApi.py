...
@testApi.get("/{user_id}", response_model=ShowUserSchema, dependencies=[Depends(RoleChecker(acces_get_usser))])
def get_user(user_id: int, db: Session = Depends(db_connection)):
    userService = UserService(db=db)
    return userService.get_item(user_id)


@testApi.post("/create", response_model=ShowUserSchema, status_code=status.HTTP_201_CREATED)
def create_user(user: UserSchema, db: Session = Depends(db_connection)):
    userService = UserService(db=db)
    return userService.create_item(user)


@testApi.patch("/{user_id}", response_model=ShowUserSchema, dependencies=[Depends(RoleChecker(acces_update_user))])
